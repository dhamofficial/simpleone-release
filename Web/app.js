...
app.factory('DataService',function($q,$http){
  ...
  service.init=function(p){
    if(p)config=p;
    config.url='http://localhost/Service/api/master';
  }
  service.getdata=function(){
     var d = $q.defer();
     
        /*
        $http({url:config.url}).then(function () { d.resolve('Release detail added.'); });
        */
        var response = $http({
...
app.controller('MainCtrl', function($scope,localStorageService,$location,$routeParams,growl,ReleaseService,DataService) {
  var remotedb=true;
  $scope.Items=[];
  ...
  $scope.init=function(){
    ...
    var master = ReleaseService.master();
    $scope.Customers = master.Customers;
    $scope.ReleaseTypes = master.ReleaseTypes;
    $scope.Environments = master.Environments;
